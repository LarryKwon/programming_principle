...
struct Find_by_name{
  string name;
  Find_by_name(string name):name(name){}
...
void f1()
{
    ...
    vector<Item> vi;
    const string ifname = "chapter21_drill_in.txt";
...
template<typename T>
struct Map_add{
  T operator()(T val, const pair<string,T>& p){
    return val + p.second;
  }
};
...
{
    // 3.1
    const string fname = "chapter21_drill_in2.txt";
